...
class DynamicHook:
    ...
    def _compile_function(self) -> Callable:
        ...
        try:
            ...
            debug_logger.log_error("dynamic_hook", "compile_function",
    ...
    def process(self, request: RequestInfo) -> HookAction:
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                debug_logger.log_error("dynamic_hook", "process",
...
class DynamicHookSystem:
    ...
    async def setup_interception(self, tab, instance_id: str):
        ...
        try:
            ...
            for hook in all_hooks:
                ...
                if stage == 'response':
                    pattern = uc.cdp.fetch.RequestPattern(
                        ...
                        resource_type=getattr(uc.cdp.network.ResourceType,
                                              resource_type.upper()) if resource_type else None,
            ...
            debug_logger.log_info("dynamic_hook_system", "setup_interception",
    ...
    async def _on_request_paused(self, tab, event, instance_id: str):
        ...
        try:
            ...
            debug_logger.log_info("dynamic_hook_system", "_on_request_paused",
            ...
            debug_logger.log_error("dynamic_hook_system", "_on_request_paused",
    ...
    async def _process_request_hooks(self, tab, request: RequestInfo, event=None):
        ...
        try:
            ...
            if not matching_hooks:
                debug_logger.log_info("dynamic_hook_system", "_process_request_hooks",
                                      f"No matching hooks for {request.stage} stage: {request.url}")
                if request.stage == "response":
                    await tab.send(
                        uc.cdp.fetch.continue_response(request_id=uc.cdp.fetch.RequestId(request.request_id)))
                else:
            ...
            if request.stage == "response" and event:
                try:
                    body_result = await tab.send(
                        uc.cdp.fetch.get_response_body(request_id=uc.cdp.fetch.RequestId(request.request_id)))
                    response_body = body_result[0]  # body content
                    debug_logger.log_info("dynamic_hook_system", "_process_request_hooks",
                                          f"Retrieved response body ({len(response_body)} chars)")
                except Exception as e:
                    debug_logger.log_error("dynamic_hook_system", "_process_request_hooks",
    ...
    async def _execute_hook_action(self, tab, request: RequestInfo, action: HookAction, event=None):
        ...
        try:
            ...
            if action.action == "block":
                ...
                debug_logger.log_info("dynamic_hook_system", "_execute_hook_action",
            ...
            debug_logger.log_error("dynamic_hook_system", "_execute_hook_action",
